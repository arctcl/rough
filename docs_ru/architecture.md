...
- **Переменные:** `$имя` в action подставляется движком в момент выполнения шага;
  запись — резервное слово движка `export` (`... | export:имя`), удаление — `unexport`,
  накопление суммы — `export:имя +=` (аккумулятор).
...
через публичные API — например, регистрировать последовательности клавиш
(`rough.AddCheatRoute` — переход на страницу) и читать конфиги после готовности
вшитой папки (`rough.OnReady`). Так плагин «встраивается» в ядро, а не только
исполняется из HTML; инжектор `chch` открывает секретные страницы данными из
`chch.json`. **Async-плагины** (`async`) выполняются в своей горутине
(плагин-служба), а плагин-API (`Window/PluginKey/ThemeColor/SetVar/...`) защищён
мьютексом — его безопасно звать из фона (рейсов нет).

## 5.1 Переменные сессии

Движок хранит глобальную память — переменные (имя → строки). Инфраструктура живёт в
движке (`engine/vars.go`): `SetVar/VarLine`. Запись/удаление/накопление — резервные
слова движка `export`/`unexport`/`export:имя +=` (НЕ плагины: `... | export:имя` —
сохраняет вход и пропускает дальше, как `tee`), подстановку — движок: `$имя` / `${имя}`
в любом action подставляется значением на момент выполнения шага (`\$` — литерал,
