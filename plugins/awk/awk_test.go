...
func TestAwkFilter(t *testing.T) {
	out, err := engine.RunSteps([]string{"awk --фильтр=ERROR"}, []string{"x ok", "y ERROR 1", "z ERROR 2"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAwkFilterField(t *testing.T) {
	out, err := engine.RunSteps([]string{"awk --фильтр=ERROR --поля=3"}, []string{"x ok 0", "y ERROR 42", "z ERROR 7"})
	if err != nil {
		t.Fatal(err)
	}
