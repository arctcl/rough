...
<!-- живой столбчатый график: новый столбик каждые 2с, старые сдвигаются влево -->
<plugin pipe="emu_cpu | chart:0:100" height="14" interval="2s"/>

<!-- chart с шириной столбика 2 клетки -->
...
`chart` рисует на всей доступной зоне: ширину берёт из тайла/колонки,
высоту — из атрибута `height` на `<plugin>`. Столбики привязаны к низу.
`bars` подстраивается под ширину через `engine.Window()`: поменял
`width="50%"` на `40%` — график сам стал уже.
...
| `chart` | `… \| chart:МИН:МАКС[:ШИРИНА]` | живой столбчатый график (нужен `height`) |
