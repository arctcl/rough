...
// открывает страницу.
//
// Страницы при этом — ОБЫЧНЫЕ тайлы в tiles.json (html в tiles/), просто не в
// menu: вкладок не видно, попасть можно только секретным кодом.
//
...
// Нет файла или ошибка — плагин молча бездействует (движок работает как обычно).
func load(fsys fs.FS) {
	...
	// Для каждого кода — навигация на страницу (переход, как по вкладке).
