...
подтверждение. Юзер вводит слово, движок берёт 500 строк из `lev_tolstoy.txt`,
...
**ЧТО ХОЧУ:** выполнить команду на диапазоне адресов/значений (`[N-M]`).
**ПРИМЕР:**
```html
<button action="ssh:root:192.168.1.[1-250]:apt upgrade" output="out">Обновить все</button>
```
Диапазон `[1-250]` разворачивается в перебор: движок выполнит команду на каждом
хосте `192.168.1.1 … 192.168.1.250`, а выводы склеит в один блок. Работают и
`[a-c]` (буквы), и `[1,4,9]` (список), и несколько диапазонов сразу (произведение).
