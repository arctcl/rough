...
// Вызов: ... | chart:MIN:MAX[:WIDTH[:SECONDS[:TITLE]]]
package chart
...
var chartParams = []engine.Param{
	{Name: "min", Required: true},   // диапазон снизу
	{Name: "max", Required: true},   // диапазон сверху
	{Name: "width", Default: "1"},   // ширина столбика в клетках
	{Name: "seconds", Default: "2"}, // секунд на столбик (подпись времени)
	{Name: "title"},                 // название графика (необязательный)
}
...
var man_chart = "chart — живой столбчатый график (rolling).\n\n" +
	...
	"Параметры:\n" +
	"  MIN, MAX  — диапазон значений, обязательные (например 0:100 для CPU).\n" +
	"  WIDTH     — ширина столбика в клетках (по умолчанию 1).\n" +
	"  SECONDS   — секунд на столбик (для подписи времени, по умолчанию 2).\n" +
	"  TITLE     — название графика (в разрыве верхней линии).\n\n" +
	...
	"глотает остаток двоеточий. Микс: chart::1:2:CPU --min=0 --max=100.\n\n" +
	...
	"  <plugin pipe=\"emu_cpu | chart --min=0 --max=100 --title=CPU\" height=\"14\" interval=\"2s\"/>"
...
func init() {
	...
	rough.AddPlugin("chart", func(in []string, args []string) ([]string, error) {
		...
		lo, err1 := strconv.ParseFloat(vals["min"], 64)
		hi, err2 := strconv.ParseFloat(vals["max"], 64)
		if err1 != nil || err2 != nil || hi <= lo {
			return nil, errors.New("chart: нужен диапазон мин<макс")
		}
		colW := 1
		if v, err := strconv.Atoi(vals["width"]); err == nil && v > 0 {
			colW = v
		}
		secPer := 2
		if v, err := strconv.Atoi(vals["seconds"]); err == nil && v > 0 {
			secPer = v
		}
