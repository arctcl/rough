...
// папки. Нужны плагинам-инжекторам: в init() вшитой папки ещё
// нет (curFS ставится позже), поэтому свои конфиги они читают именно здесь.
...
// Плагин-инжектор читает тут свой конфиг и регистрирует секретные коды.
...
func Run(fsys fs.FS) (err error) {
	...
	curFS = fsys
	// Плагины-инжекторы читают свои конфиги после готовности папки.
