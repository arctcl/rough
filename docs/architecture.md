...
c:\...\rough\                  # корень = модуль rough (package rough)
  ...
  plugins\                     # ВСЕ плагины в одном месте (корень репозитория)
    plugins.go                 # агрегатор: cat + hello + nyan + ssh + curl
    cat\  hello\  nyan\        # базовые плагины (nyan — анимация)
    ssh\  curl\                # сетевые: SSH и HTTP — нативно в Go, без внешних бинарников
  ...
  README.md                    # для GitHub (не трогаем)
```
...
| `ssh` | выполнение команды по SSH (`golang.org/x/crypto/ssh`), агент + ключи `~/.ssh` |
...
1. Вёрстка: `checkbox`, `select`, `<table>`, `img` (картинки половинчатыми блоками `▀▄█`),
   скролл внутри тайла.
2. Плагины в корне `plugins/`: `grep`, `toggle`, `bars` и т.п. (сейчас `cat`/`hello`/`nyan`/`ssh`/`curl`).
3. Актуальный `README.md` (сейчас — первоначальная идея, не обновлён).
...
- `README.md` — пока не трогаем (см. roadmap).
