...
Использование (единые quick-параметры: позиционные или флаги, можно микс):
  action="ssh:user:host::cmd"             — порт по дефолту 22
  action="ssh:user:host:2222:cmd"         — порт позиционно
  action="ssh:user:host::cmd --port=2222" — порт флагом
  action="ssh:user:host::cmd --keys=ПУТЬ" — свои ключи

Аргументы:
  user    — пользователь на сервере.
  host    — адрес сервера.
  port    — порт (по умолчанию 22): пустой слот, позиционно или флаг --port.
  cmd — что выполнить (остаток через «:»).
