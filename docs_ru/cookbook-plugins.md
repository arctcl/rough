...
- **секретные последовательности клавиш** — `AddCheat` (действие) и
  `AddCheatRoute` (переход на страницу);
- **готовность движка** — `OnReady` (вшитая папка уже загружена).

### 16.1 Хук клавиш с действием: `AddCheat`
...
```go
func init() {
	// код: ↑ ↑ ↓ ↓ ← → ← → B A → вывести «GLHF mate!»
	rough.AddCheat("UUDDLRLRba", "cat GLHF mate!")
}
```
...
выполняет `action` (обычно это пайп — как кнопка `action`). Результат попадает
в статус-строку.

### 16.2 Хук клавиш с переходом: `AddCheatRoute`

Тот же механизм, но вместо выполнения действия — **переход на страницу**:

```go
rough.AddCheatRoute("ps+", "/ps")
```

Страница при этом — **обычный тайл** со своим html (в `tiles/`). Секретные
страницы можно задавать **программно через `AddPage`** (см. ниже) — тогда
`tiles.json` не трогаем вовсе, страниц нет ни в меню, ни в списке: попасть на
неё можно только секретным кодом. Это и есть «секретная» страница: без знания
кода туда попасть почти невозможно. На самой странице — обычные плагины,
...
### 16.3 Инжектор `chch` (конфиг-драйв) и хук `OnReady`
...
- Делается через **публичные API движка** (`rough.AddCheat` / `rough.AddCheatRoute`
  / `rough.AddPage` / `rough.OnReady` → `engine.*`), а не «взломом» кода.
