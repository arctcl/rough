...
Сейчас есть один хук — **секретные последовательности клавиш**
(`engine.AddCheat`).

### 16.1 Хук клавиш: конами-код

Плагин регистрирует последовательность клавиш и действие, которое движок
выполнит при её вводе:

```go
// plugins/konami/konami.go
package konami

import "github.com/arctcl/rough"

func init() {
	// плагин, который выводит приветствие
	rough.AddPlugin("glhf", func(in []string, args []string) ([]string, error) {
		return []string{"GLHF mate!"}, nil
	})
	// конами-код: ↑ ↑ ↓ ↓ ← → ← → B A → выполнить glhf
...
выполняет `action` (обычно это плагин, но может быть любой action).

### 16.2 Как это работает

`engine.AddCheat(seq, action)` добавляет последовательность в реестр. В
`handleKey` движок кодирует каждую клавишу, держит буфер последних нажатий и
проверяет его хвост на совпадение. При совпадении — выполняет action и сбрасывает
буфер. Плагин при этом остаётся обычным Go-пакетом: `init()` вызывает
`rough.AddCheat`, подключается одной строкой в агрегаторе.

### 16.3 Суть инъекции

- Плагин не только «вызывается из HTML», но и **расширяет движок**.
- Делается через **публичные API движка** (`rough.AddCheat` → `engine.AddCheat`),
  а не «взломом» кода.
- Удобно для: пасхалок, глобальных хоткеев, отладки.
