...
Переменная, записанная через `export`, **доступна всегда и отовсюду**: из любой
кнопки, поля, тайла и даже из **другого `&&`-пайпа**. `$имя` подставляется в
момент выполнения шага, а не при разборе действия, поэтому `export` из раннего
пайпа успевает записать значение до того, как поздний его прочитает.
