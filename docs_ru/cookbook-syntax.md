...
## 🚂 Пример-монстр: весь синтаксис в одном поле ввода

Один `<input>` — сразу и `&&`, и `|`, и `$in`, и переменные, и `line:N-M`, и
ssh, и подтверждение. Юзер вводит слово; движок узнаёт длину `lev_tolstoy.txt`,
читает его **кусками по 500 строк** (`line:0-499`, `line:500-999`, …), считает
вхождения и отправляет по ssh в докер столько паровозиков `sl`, сколько нашлось.
Всё — в одном элементе:

```html
<input action="
  clear
  && cat:lev_tolstoy.txt | wc:lines | export:ln_sum
  && cat:lev_tolstoy.txt | line:0-499 | grep:$in | wc:lines | export:count
  && cat:lev_tolstoy.txt | line:500-999 | grep:$in | wc:lines
  && cat:lev_tolstoy.txt | line:1000-1499 | grep:$in | wc:lines
...
- **`cat:lev_tolstoy.txt | wc:lines | export:ln_sum`** — узнать, сколько всего
  строк, и запомнить в `$ln_sum`.
- **`cat:lev_tolstoy.txt | line:0-499 | grep:$in | wc:lines | export:count`** —
  взять **кусок** строк с 0-й по 499-ю, найти слово, посчитать, запомнить в
  `$count`. Следующие `&&`-куски — `line:500-999`, `line:1000-1499` и т.д. Так
  файл обрабатывается **по 500 строк**, не держа в памяти всё сразу.
  Чтобы перебрать **все** куски, старты задают диапазоном с шагом:
  `[0-$ln_sum:500]` → `0,500,1000,…`.
...
Если хочется ограничиться **только первыми 500 строками** — убери лишние `&&`
куски и оставь один `line:0-499`.
