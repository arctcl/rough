...
## 12. Переменные: API для плагина

В HTML переменные собирает сам движок (`export`/`$имя` — см. [guide, раздел 9](guide.md)
и [reference, раздел 5](reference.md)). Плагину они обычно не нужны, но если
захочешь сам записать или прочитать переменную — вот API движка:
...
Эти функции потокобезопасны — их можно звать из async-горутины плагина.

## 13. Пайпы — это движок, не плагин

Плагин не знает про пайпы: он получает `in` (строки с прошлого шага) и возвращает
строки, а склейку шагов в цепочку `|`, подтверждение `| confirm` и `&&` делает
движок. Как это работает со стороны HTML — [guide, разделы 7–8](guide.md).
