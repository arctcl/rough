...
`<plugin>` сам себя обновляет по таймеру — без кликов. Интервал — через
`interval` (не задан → дефолт 2 секунды).

```html
<!-- часы, обновление раз в секунду -->
<div align="center"><plugin name="clock" interval="1s"/></div>

<!-- живой столбчатый график: заголовок в верхней линии, новый столбик справа -->
...
`chart` — живой столбчатый график с осями (ось справа). Параметры — гибрид:
двоеточиями по порядку (`chart:МИН:МАКС[:ШИРИНА[:СЕКУНД[:ЗАГОЛОВОК]]]`) или
`--флаги` (`chart --мин=0 --макс=100 --заголовок=CPU`), можно микс.
Последний параметр — название графика (в разрыве верхней линии), он глотает
остаток двоеточий. Ширину берёт из тайла/колонки, высоту — из атрибута
`height` на `<plugin>`. Столбики привязаны к низу, новые появляются справа
у оси, старые уходят влево.
...
Клик или Enter (фокус + Enter) открывают меню **справа от кнопки select** —
левый верхний угол всегда строго наискосок от кнопки, независимо от того,
куда кликнули. Выбор дописывается к вызову через `:`. Esc или клик мимо —
закрыть. Можно кликнуть вариант мышью.
...
Вариант с подменю пишется как `Метка [вложенные:варианты]` — дети в квадратных
скобках, разделитель `:`, вложенность любая. Подменю открывается **справа от
родительского меню** (клавиша →, Enter или клик по варианту), верхняя граница
вровень с родителем. ← или Esc — вернуться/закрыть. Такие варианты помечаются
стрелкой `▶`.

```html
<select action="set:/etc/app.conf:theme" options="Тёмная:Светлая:Цветная [Красная:Зелёная:Синяя [Тёмно-синяя:Небесная]]" label="Тема"/>
```

### Прокрутка длинного списка

Высота меню ограничена **50% высоты экрана**. Если варианты не влезают — список
прокручивается тремя способами:

- **колесо мыши** над списком;
- **мышью** (зажать левую кнопку и тянуть вверх/вниз);
- **стрелками** (курсор подтягивает список за собой).

У прокручиваемого списка справа — полоса прокрутки.
...
| `<select action="…" options="…" label="…"/>` | выпадающий список (подменю — `Метка [дети:…]`) | `<select action="set:…" options="a:b:c"/>` |
