...
Бери пример и вставляй в свой `tiles.json` / `tiles/*.html`.
...
**ЧТО ХОЧУ:** выполнить команду на диапазоне адресов (`[N-M]`).
**ПРИМЕР:**
```html
<button action="ssh:root:192.168.1.[1-250]:apt upgrade" output="out">Обновить все</button>
```
`[1-250]` разворачивается в перебор: команда выполняется на каждом хосте
`192.168.1.1 … 192.168.1.250`, а выводы склеиваются в один блок.

**ЧТО ХОЧУ:** диапазон с шагом (`[N-M:S]`) — например, брать кусок по 500 строк.
**ПРИМЕР:**
```html
<button action="cat:data.log | line:0-500 | grep:$in | wc:lines">первый кусок</button>
<button action="ssh:root:10.0.0.[0-1000:100]::ping">каждая сотая</button>
```
`[0-1000:100]` → `0,100,200,…,1000` (шаг через двоеточие). Вместе с `line:N-M`
(строки с N по M) можно обрабатывать файл кусками. Диапазон может содержать
переменную: `[0-$n:500]` раскрывается в момент выполнения шага (после подстановки
`$n` в число) — так число, посчитанное `wc`, превращается в перебор.

**ЧТО ХОЧУ:** повторить следующие команды N раз (`loop:N`).
**ПРИМЕР:**
```html
...
```
`loop:N` — ключевое слово: **просто запускает остальные шаги пайпа N раз** и
склеивает выводы. Здесь `$count` уже раскрыт в число (например, `loop:7`), так
что `ssh:...:sl` (паровозик) поедет ровно 7 раз. `loop:1` — как без цикла.
Диапазон `[0-$ln_sum:500]` держит переменную: он раскрывается в RunSteps на
момент выполнения шага (после подстановки `$ln_sum` в число), а не на старте,
когда переменная ещё пуста.

**ЧТО ХОЧУ:** диапазон букв.
**ПРИМЕР:**
```html
<button action="ssh:root:srv[a-c]::uptime">srv a..c</button>
```
→ выполнится на `srv a`, `srv b`, `srv c`.

**ЧТО ХОЧУ:** список значений через запятую.
**ПРИМЕР:**
```html
<button action="ssh:root:10.0.0.[1,4,9]::reboot">Перезагрузить 1,4,9</button>
```

**ЧТО ХОЧУ:** несколько диапазонов сразу (декартово произведение).
**ПРИМЕР:**
```html
<button action="ping:192.168.[1-2].[1-2]">4 адреса</button>
```
→ `192.168.1.1`, `1.2`, `2.1`, `2.2`.

> Содержимое в `[...]`, которое **не похоже** на диапазон или список (например
> `[foo]`), не трогается — остаётся как есть.
...
всегда и отовсюду**: из любой кнопки, поля, тайла и даже из **другого
`&&`-пайпа**. `$имя` подставляется в момент выполнения шага, а не при разборе
действия, поэтому `export` из раннего пайпа успевает записать значение до того,
как поздний его прочитает.
...
**ЧТО ХОЧУ:** использовать переменную из **другого** `&&`-пайпа.
**ПРИМЕР:**
```html
<button action="cat:data.log | wc:lines | export:n
  && hello:строк $n" >Сколько строк</button>
```
`export:n` записал число в первом пайпе, второй пайп `hello:строк $n` его
читает — `$n` подставилось уже на этапе выполнения.

**ЧТО ХОЧУ:** удалить переменную (`unexport` — антипод `export`).
**ПРИМЕР:**
```html
<button action="... | export:tmp | ... | unexport:tmp">временная</button>
```
После `unexport:имя` переменная `$имя` больше нигде не подставляется (пустая
строка). Удобно чистить временные значения по окончании действия.
...
**ЧТО ХОЧУ:** накопить сумму по кускам файла (`export:ИМЯ +=` — аккумулятор).
Обычный `export:имя` **перезаписывает** переменную при каждом вызове: в
многошаговом пайпе останется только последнее число, а суммы по кускам не
будет. Форма `export:ИМЯ +=` заставляет движок **прибавлять** число к текущему
значению переменной, а не заменять его. Число берётся из текущего вывода пайпа
(кусок) либо задаётся явно после оператора (`export:count += 5`). Движок сам
складывает — плагин `wc` только считает строки одного куска.
**ПРИМЕР:**
```html
<button action="clear
...
```
Три куска по 500 строк: `$count` после каждого `export:count +=` растёт, а не
перезаписывается, и в конце содержит общую сумму. (Напомним: `line` читает файл
целиком — «куски» лишь разбиение на итерации, не экономия памяти.)
