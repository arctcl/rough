...
// движок ПЕРЕХОДИТ на страницу (роут) — навигация, как по вкладке.
...
// AddPage регистрирует страницу (роут → тайлы) программно: она добавляется в
// общий список страниц при старте Run (страницы могут жить не только в tiles.json).
...
// папки /rough (в init() вшитой папки ещё нет). Колбэк получает саму папку.
func OnReady(fn func(fs.FS)) { engine.AddOnReady(fn) }
