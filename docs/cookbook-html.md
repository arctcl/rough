...
<!-- японские свечи: вход — OHLC (open high low close), бычья зелёная, медвежья красная -->
...
Последний параметр — название графика (в разрыве верхней линии). Для свечей
указывай непрерывный ряд (open = close предыдущей), чтобы не было гепов.
...
| `chart` | `… \| chart:МИН:МАКС[:bars\|japanse[:ШИРИНА[:СЕКУНД[:ЗАГОЛОВОК]]]]` | живой график с осями (столбики или свечи, нужен `height`) |
