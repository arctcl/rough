...
Страница при этом — **обычный тайл** в `tiles.json` со своим html (в `tiles/`),
просто **не в `menu`**: вкладок не видно, попасть на неё можно только секретным
кодом. Это и есть «секретная» страница: без знания кода туда попасть почти
невозможно. На самой странице — обычные плагины, например сразу выводящееся
приветствие:
...
Вручную звать `AddCheatRoute` для каждого кода неудобно — это хардкод. Вместо
этого есть плагин-инжектор **`chch`**: он читает конфиг `chch.json` из вшитой
папки `/rough` и сам регистрирует все коды как переходы на страницы.

`chch.json` (стиль как у тем: сверху заголовок-описание, внутри плоский список
кодов, без вложенных заголовков):
