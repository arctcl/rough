...
переменная — пусто. Чтобы **накопить** сумму по кускам (а не перезаписать),
используется аккумулятор `export:имя +=`: число берётся из текущего вывода пайпа
или задаётся явно (`export:count += 5`) — движок прибавляет его к значению
переменной.
