...
`chart` — живой столбчатый график с осями (ось справа). Параметры:
`chart:МИН:МАКС[:ШИРИНА[:СЕКУНД[:ЗАГОЛОВОК]]]`. Последний параметр —
название графика (в разрыве верхней линии). Ширину берёт из тайла/колонки,
высоту — из атрибута `height` на `<plugin>`. Столбики привязаны к низу,
новые появляются справа у оси, старые уходят влево.
