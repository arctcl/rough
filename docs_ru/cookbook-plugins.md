...
- **секретные последовательности клавиш** — `AddCheat` (действие/пайп);
...
```go
func init() {
	// конами-код: ↑ ↑ ↓ ↓ ← → ← → B A → вывести «GLHF mate!»
...
Вручную звать `AddCheat` для каждого кода неудобно — это хардкод. Вместо
этого есть плагин-инжектор **`chch`**: он читает конфиг `chch.json` из вшитой
папки `/rough` и сам регистрирует все коды как действия.
...
просто секретных кодов нет. Это и есть «внедрение без взлома»: всё через
публичные API движка.

### 16.3 Суть инъекции

- Плагин не только «вызывается из HTML», но и **расширяет движок**.
- Делается через **публичные API движка** (`rough.AddCheat` / `rough.OnReady`
  → `engine.*`), а не «взломом» кода.
- Удобно для: пасхалок, глобальных хоткеев, секретных хоткеев-команд, отладки.
