...
c:\...\rough\                  # корень = модуль rough (package rough)
  ...
  plugins\                     # ВСЕ плагины в одном месте (корень репозитория)
    plugins.go                 # агрегатор: cat + hello + nyan
    cat\  hello\  nyan\        # каждый плагин — пакет (nyan — анимация)
...
1. Вёрстка: `checkbox`, `select`, `<table>`, `img` (картинки половинчатыми блоками `▀▄█`),
   скролл внутри тайла.
2. Плагины в корне `plugins/`: `grep`, `toggle`, `bars` и т.п. (сейчас `cat`/`hello`/`nyan`).
