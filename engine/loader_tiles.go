...
// json-теги нужны инжекторам, которые задают страницы в своём конфиге.
type Tile struct {
	ID   string `json:"id"`
	X    string `json:"x"`
	Y    string `json:"y"`
...
// живут не в tiles.json, а в конфиге инжектора и добавляются в
// общий список страниц при старте Run.
var extraPages = Pages{}

// AddPage регистрирует страницу (роут → тайлы) программно. Используется
// инжекторами для секретных страниц: страница не трогает tiles.json,
