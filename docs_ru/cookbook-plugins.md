...
Страница при этом — **обычный тайл** в `tiles.json` (html в `tiles/`), просто
**не в `menu`**: вкладок не видно, попасть на неё можно только секретным кодом.
Это и есть «секретная» страница: без знания кода туда попасть почти невозможно.
...
папки `/rough` и сам регистрирует все коды.
