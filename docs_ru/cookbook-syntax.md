...
Один `<input>` — сразу и `&&`, и `|`, и `$in`, и переменные, и ssh, и
подтверждение. Юзер вводит слово, движок ищет его по всему `lev_tolstoy.txt`,
считает вхождения, и отправляет по ssh в докер столько паровозиков `sl`, сколько
их нашлось. Всё — в одном элементе:
