...
(`rough.AddCheat` — действие, `rough.AddCheatRoute` — переход на страницу) и читать
конфиги после готовности вшитой папки (`rough.OnReady`). Так плагин «встраивается»
в ядро, а не только исполняется из HTML; инжектор `chch` открывает секретные страницы
данными из `chch.json`. **Async-плагины** (`async`) выполняются в своей горутине
